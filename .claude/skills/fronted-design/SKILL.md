...
- Primary accent: a single confident color - indigo/violet (`#6366F1`), blue (`#3B82F6`), or similar. Pick one and stick with it. Avoid green, red, or amber as the primary accent - those are reserved as semantic colors below, and reusing one as the brand color makes it ambiguous whether a highlighted element means "positive/negative" or just "important."
...
(Pin a specific version rather than `@latest` - an unpinned CDN import can pull a breaking change on any future page load. Bump the version deliberately when needed.)
...
**Template:** `templates/partials/add_expense_modal.html` - extends nothing, included via `{% include %}`. Uses a `.modal` overlay pattern already in the project's existing stylesheet if present.
